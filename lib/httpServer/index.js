...
class HttpServer {
    constructor(node, blockchain, operator, miner) {
        ...
        this.app.post('/miner/mine', (req, res, next) => {
            // Add to mine requests
            MineRequests.push({taskId: miner.taskId, timestamp: Date.now(), resCount: 0});

            let nodeId = blockchain.dbName;
            let totalNodes = node.peers.length + 1;

            miner.mine(req.body.rewardAddress, req.body['feeAddress'] || req.body.rewardAddress, nodeId, totalNodes)
                .then((newBlock) => {
        ...
        this.app.put('/miner/newMineRequest', (req, res) => {
            ...
            node.sendMineResult('http://localhost:3001', taskId);
        });

        this.app.put('/miner/newMineResult', (req, res) => {
            let taskId = req.body.taskId;
            
            const lastIndex = MineRequests.length - 1;
            MineRequests[lastIndex].resCount += 1;

            if (MineRequests[lastIndex].resCount == node.peers.length) {
                console.log('Need to do average for task ' + taskId);
            }
        });
    ...
    getMineRequest(ts) {
        for (let i = 0; i < MineRequests.length; i++) {
            if (MineRequests[i].ts == ts) {
                return MineRequests[i];
            }
        }
    }
