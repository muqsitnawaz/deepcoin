...
class Node {
    ...
    hookBlockchain() {
        ...
        this.blockchain.emitter.on('newMineRequest', (jsonBlock, taskId, ts) => {
            this.broadcast(this.sendMineRequest, jsonBlock, taskId, ts);
        });
    }
    ...
    sendMineRequest(peer, reqBlock, taskID, ts) {
        const URL = `${peer.url}/miner/newMineRequest`;
        console.info(`Posting mine request to: ${URL}`);
        return superagent
            .put(URL)
            .send({block: reqBlock, taskId: taskID, timestamp: ts})
            .catch((err) => {
                console.warn(`Unable to post mine request to ${URL}: ${err.message}`);
            });
    }

    sendMineResult(peerurl, taskID, ts) {
        const URL = `${peerurl}/miner/newMineResult`;
        console.info(`Posting mine result to: ${URL}`);
        return superagent
            .put(URL)
            .send({taskId: taskID, timestamp: ts})
            .catch((err) => {
                console.warn(`Unable to post mine result to ${URL}: ${err.message}`);
            });
    }
