...
class Miner {
    ...
    mine(rewardAddress, feeAddress, nodeId, totalNodes) {
        ...
        this.blockchain.emitter.emit('newMineRequest', baseBlock, this.taskId);
    ...
    // Processes a mine request
    newMineRequest(reqblock, taskId, nodeId, totalNodes) {
        execFile('python3', ['./bin/fetchData.py', taskId, nodeId, totalNodes], (error, stdout, stderr) => {
            if (error) {
                console.error(error)
            }
            console.info('Fetching data from repo for task '+taskId)

            this.runTask(nodeId, taskId);
            this.incrTaskID();
        });
    }
    ...
    // Runs a task given by taskId
    runTask(nodeId, taskId) {
        console.info('Training task '+taskId);

        let scriptPath = './data/'+nodeId+'/tasks/'+taskId+'/model/model.py'
        execFile('python3', [scriptPath, '-e', '1', '-n', nodeId, '-k', taskId], (error, stdout, stderr) => {
            if (error) {
                console.error(error)
            }

            console.log('Executing training script')
            console.log(stdout);
        });
    }
