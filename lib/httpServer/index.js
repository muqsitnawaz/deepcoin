...
class HttpServer {
    constructor(node, blockchain, operator, miner) {
        ...
        this.app.post('/miner/mine', (req, res, next) => {
            ...
            let totalNodes = node.peers.length;
            let ts = Date.now();
        ...
        this.app.put('/miner/newMineRequest', (req, res) => {
            ...
            for (let i = 0; i < MineRequests.length; i++) {
                if (MineRequests[i].timestamp == ts) {
                    count = count + 1;
                }
            }
            ...
            if (count == 0) {
                ...
                node.sendMineResult('172.31.17.50:3001', taskId, ts);
            } else {
                console.info('Discarding duplicate mine request');
            }
        });
                ...
                if (MineRequests[i].taskId == ti && MineRequests[i].timestamp == ts) {
                    ...
                    if (MineRequests[i].resCount == node.peers.length) {
                        ...
                        const average = async() => {
                            ...
                            await snooze(700000);
                            console.info('Run average script: task ' + ti + ', timestamp ' + ts);

                            execFile('python3', ['./bin/avg.py', '-k', ti], (error, stdout, stderr) => {
                                if (error) {
                                    console.error(error)
                                }
